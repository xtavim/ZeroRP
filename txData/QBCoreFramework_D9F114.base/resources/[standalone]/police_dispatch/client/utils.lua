function ShootingDispatch(location, coords, gender, weapon, vehicleName, vehicle, jobs)
    if vehicle ~= 0 then
        TriggerServerEvent("police_dispatch:server:shootingDispatch", "Drive By In Progress", "10-60", location, coords, gender, weapon, vehicleName, vehicle, jobs)
    else
        TriggerServerEvent("police_dispatch:server:shootingDispatch", "Shooting In Progress", "10-11", location, coords, gender, weapon, vehicleName, vehicle, jobs)
    end
end
...
function SendDispatch(title, code, blipSprite, jobs)
    ...
    TriggerServerEvent("police_dispatch:server:customDispatch", title, code, location, coords, gender, vehicleName, vehicle, weapon, blipSprite, jobs)
end

RegisterNetEvent("police_dispatch:SendDispatch", function(title, code, blipSprite, jobs)
    local title = title or "Placeholder"
    local code = code or "10-11"
    local blipSprite = blipSprite or 1
    local ped = PlayerPedId()
